...
|1|排程建立|4634|帳戶已經登出|
|11|檔案建立|4647|帳戶已經登出|
|15|檔案建立Stream Hash|4648|帳戶成功登入|
|261|連線接收|4657|修改Registry value|
|1100|Windows已停止事件紀錄|4697|安裝新的服務|
|1101|事件在傳輸過程被丟棄|4688|已建立新的處理程序|
|1102|事件紀錄檔已被清除|4720|使用者帳戶已被成功創建|
|1149|授權成功|4724|您嘗試重設帳戶密碼|
|4104|執行Remote Command|7034|服務無預期關閉|
|4624|帳戶成功登入|7036|服務啟動或是關閉|
|4625|帳戶登入失敗|7040|服務啟動模式改變|
...
* 第一欄 : 檔案的類型與權限
  ...
  * 接下來的字元，以三個為一組，且均為 rwx 的三個參數的組合 
    * [ r ] 為可讀(read)   
    * [ w ] 為可寫(write)    
    * [ x ] 為可執行(execute)    
    * [ - ] 為沒有權限
    
    第一組為檔案擁有者的存取權限，第二組為檔案擁有者所屬群組成員的存取權限，第三組為其他使用者的存取權限
